...
url = "http://ask.globecancer.com/tags/all.html"   # 抗癌帮所有热点展开后网页
...
views = soup.find('h2', class_='h4')  # 找到展开后所有页面中相关内容
# print(type(views))  # <class 'bs4.element.Tag'>
print(views.find_all('h2'))  # 找到二级标题
print(views)
a = views.find_all('li')
for i in a:
    b = BeautifulSoup(str(i), "html.parser")
    print(b.select_one('.mr10')['href'])
    # print(b.select_one('.tagPopup').text.strip(), b.select_one('.tag')['href'])

    # print(a.get('href'))  # 获取相对应的链接

# title = soup.find('h1', class_='title-article').text.strip()  # 要记住提取文字的方法,strip()是指去掉两旁空格

title = soup.find('li', class_='h4').text.strip()  # 要记住提取文字的方法,strip()是指去掉两旁空格
# link1 = soup.find('a', class_='h4').get_text
print(title)
