...
url = "https://www.douyin.com/"  # 要爬取的抗癌帮列表网页
...
# views = soup.find('div', class_='news-item stream__item clearfix')  # 找到列表页面中所有的相关内容
# views = soup.find('div', class_='news__item-title')  # 找到列表页面中所有的相关内容
views = soup.find('div', class_ ='_4IVa9BUd')  # 找到列表页面中所有的相关内容
print(views)
# print(type(views))  # <class 'bs4.element.Tag'> 子迭代类型也是
# print(views.find_all('li'))  # 找到二级标题

# a = views.find_all('news__item-meta')
a = views.find_all('div')

"""
class="mzZanXbP" 对应的标题
canonical  链接对应的标签
h7qyKhTd 发布时间对应的  或者是div HF-f8Lg-
